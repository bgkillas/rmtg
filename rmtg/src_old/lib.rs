...
pub fn start() -> AppExit {
    ...
    let mut app = App::new();
    #[cfg(feature = "mic")]
    let mut stream_handle = DeviceSinkBuilder::open_default_sink().unwrap();
    #[cfg(feature = "mic")]
    stream_handle.log_on_drop(false);
    #[cfg(feature = "mic")]
    let sink = Player::connect_new(&stream_handle);
    app.add_plugins((
        DefaultPlugins
            .set(WindowPlugin {
...
impl<'w> Focus<'w> {
    ...
    pub fn mouse_lock(&self) -> bool {
        self.hover_map
            .values()
            .all(|a| a.keys().all(|e| e.to_bits() != u64::from(u32::MAX)))
    }
}
